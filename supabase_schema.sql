...
create policy "환자 본인 조회"
  ...
  using (
    id in (select patient_id from public.user_roles where id = auth.uid())
  );
...
create policy "환자 본인 일지 관리"
  ...
  using (
    patient_id in (select patient_id from public.user_roles where id = auth.uid())
  );
...
-- ============================================================
-- 5. 테스트용 환자 계정 등록 방법 (실행 후 아래 주석 참고)
-- ============================================================
-- 1) Supabase Authentication > Users > "Add user" 클릭
--    Email: {등록번호}@patient.local  (예: 20240001@patient.local)
--    Password: 원하는 비밀번호
--    → 생성 후 user_id(UUID) 복사
--
-- 2) 아래 SQL로 환자 및 user_roles 등록:
--
