...
export async function GET() {
  ...
  const [patientRes, todayDiaryRes, yesterdayDiaryRes, prescriptionRes, settingsRes] =
    await Promise.all([
      ...
      supabase.from('sleep_diary').select('id, herbal_morning, herbal_lunch, herbal_evening, herbal_bedtime, western_morning, western_lunch, western_evening, western_bedtime').eq('patient_id', patientId).eq('diary_date', today).maybeSingle(),
      ...
      supabase.from('settings').select('push_enabled, diary_remind, med_alarm, qna_alarm').eq('id', user.id).maybeSingle(),
    ])
  ...
  return NextResponse.json({
    ...
    settings: settingsRes.data ?? { push_enabled: false, diary_remind: false, med_alarm: false, qna_alarm: false },
  })
}
