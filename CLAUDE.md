...
> ⚠️ `user_roles` 테이블에 INSERT RLS 정책 없음 — 환자 등록 시 반드시 `adminClient`(service_role)로 삽입해야 함. (`/api/admin/patients/route.ts` 에서 이미 처리됨)
...
├── app/
│   ├── layout.tsx                        # 루트 레이아웃
│   ├── page.tsx                          # / → 역할별 리다이렉트 (비로그인→/login, admin→/admin/dashboard, 환자→/home)
...
│   │   ├── settings/page.tsx             # 설정 탭 (비밀번호 변경 + 로그아웃)
│   │   └── qna/
│   │       ├── page.tsx                  # 문의 목록 / 작성
│   │       └── [id]/page.tsx             # 문의 상세
│   │
...
│   │   └── (dashboard)/
│   │       ├── layout.tsx                # 관리자 레이아웃 (사이드바 + 역할 검증 + 비-admin signOut)
...
│           ├── patients/route.ts         # GET/POST: 환자 목록/등록 (adminClient로 user_roles 삽입)
...
│
├── components/
│   ├── layout/BottomTabBar.tsx           # 환자 하단 탭바 (6탭: 홈/기록/처방/자가진단/문의/설정)
...
│
├── middleware.ts                          # 인증 보호 (비로그인→/login, /admin/*→/admin/login)
...
- [x] 환자 등록 (`/admin/patients/new`) — Auth 계정 생성 + DB 연동 (adminClient로 user_roles 삽입)
...
- [x] 관리자 계정 생성 (`admin@clinic.com`)
- [x] PWA manifest (`public/manifest.json` + SVG 아이콘)
...
- [ ] 관리자 수면장애 진단 CRUD (API 3개 + UI)
- [ ] 처방 수정·삭제 UI (API는 구현됨)
- [ ] 관리자에서 환자 ISI 조회
- [ ] 환자 목록 페이지네이션
- [ ] 커스텀 에러 페이지 (`error.tsx`, `not-found.tsx`)
